...
| 歌单/专辑「加入库」入库前库内匹配（`remoteImport` → `libraryMatch.matchSongsToLibrary`，v2.3.8+） | 门禁核实后直接 `importOnlineSongs` 入库——同一首歌(已在曲库)因指纹含平台外部 id(榜单/歌单两接口 id 不同)重复插一行 source=本平台 的 web 行 | 门禁核实后先 `matchSongsToLibrary` 在曲库找已有行(归一化标题精确 + 歌手互相包含[硬] + 时长±5s/专辑一致[软]，同榜单同步 matchLocal 阈值；同分并列本地/WebDAV 优先、时长差最小次之)，命中的歌单条目直接绑旧行、不再插新行；未命中的才入库。与榜单同步「先匹配库、缺了才进」行为对齐，曲库不涨重复行。仅 `group_key` 非空的行可被匹配(同曲多源组默认开启，关闭分组的存量行回退原行为) |
...
> 🔴 该回环端点是 **ffmpeg 的专用喂料口**，**不是给播放器/设备的出流路径**。
> 自音频流水线改造（D9）起，**任何对端（客户端 / Web / DLNA / 音箱）都不再直连这个端点拿原始字节**，
> 一律走服务端实时管道出流；DLNA 的 `?raw=1` 直透出流分支删除。
...
> **① 解码 → PCM buffer → ② 出流 ffmpeg（响度标准化 → DSP → 交叉淡入 → 限制器）→ ⑥ 按通道编码**，
> 客户端 / Web / DLNA 三条 HTTP 链路同样经过该管道（**不再是原样直出，也不再有直传分支**）。
> 关闭开关只等于「滤镜链为空」，**不回到绕过管道**。
> 权威定义见 `docs/audio-pipeline-plan.md`（§0、§3.1、§4、D9/D10）；两者冲突时以后者为准。
> 分阶段任务与实时进度见 `docs/audio-pipeline-progress.md`（唯一任务真相源，每完成一小项即回填状态）。
...
> 收到新需求（新功能 / 修 bug / 重构）后，**禁止直接动手写代码**。必须先走完下面四步对齐流程，等开发者确认后再执行：
>
> 1. **复述理解**：用中文、大白话，把需求复述一遍——**是做什么的、给谁用、期望什么结果**。有不清楚的地方，**必须先列出来问，不许猜**；宁可多问一句，也不许带偏理解往下走。
> 2. **查看代码**：阅读对应实现，说清现状与需求的关系，标注**涉及文件 + 上下游影响面**（对齐第七章 7.2）。
> 3. **提出方案**：给出实现步骤、关键取舍、需要开发者拍板的点，仍用大白话，不堆术语。
> 4. **等待确认**：停在方案阶段，等人明确说「理解无误 / 可以动手」才算数。**开发者确认前不得改动任何代码**；若开发者修改或补充需求，重新走一遍再执行。
>
