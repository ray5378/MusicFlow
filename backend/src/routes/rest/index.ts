...
// 各分区具体内容仍由对应接口独立提供(随机歌曲 / 最近更新 / 推荐 / 插件推荐 /
// 平台推荐),分区数据为空时客户端按内容自行隐藏,清单里的 visible 由服务端决定
// 是否开放该分区 —— 客户端与服务端完全解耦,增删分区、调整排序只在服务端发生。
// 客户端另有用户本地布局(手动排序 + 显隐)覆盖本清单顺序/可见性;客户端不显示
// 的分区不会拉取对应数据接口。客户端渲染层对推荐两模块做定序归一:固定
// 「平台推荐」(local-recommend,本地库随机歌单)在「插件推荐」(platform-recommend,
// 插件提供方)之前 —— 本清单 sortOrder 与该归一保持一致。
// 可见性判定以本模块可直接查询的库状态为依据:
//  - 随机歌曲/平台推荐(本地随机)/为你推荐/最近更新:依赖本地音库存在歌单(或歌曲);
//  - 插件推荐:依赖已从平台导入的歌单(sourceUrl 非空)。
restRoutes.get("/api/v1/home/sections", permMiddleware(PERM.LIBRARY_BROWSE), (c) => {
  ...
  // 推荐模块新定位:「平台推荐」= local-recommend(本地库),"插件推荐" =
  // platform-recommend(插件提供方,原「平台推荐」),前者在前。
  const sections = [
    ...
    { key: "local-recommend", title: "平台推荐", sortOrder: 4, visible: playlistCount > 0 },
    { key: "platform-recommend", title: "插件推荐", sortOrder: 5, visible: importedCount > 0 },
