  ...
  configSchema: [
    ...
    },
    {
      ...
      options: [
        ...
        { label: "25 秒", value: "25000" },
        { label: "30 秒(最抗卡顿)", value: "30000" },
      ],
      help:
        "起播/卡顿恢复后,先把设备缓冲灌到这个水位再按实时速率推 —— 这就是设备能吸收的抖动与停顿窗口,越大越抗卡顿。\n" +
        "**本值是「期望水位」,实际水位还会按设备自己宣告的缓冲容量自动钳制**:服务端读 `client/hello` 里的 `buffer_capacity`(就是 ESPHome 的 `buffer_size`,单位字节,真机实测 1.6MB),乘 0.6 的安全余量后除以实测压缩码率,得到设备装得下的最长秒数,取更小的那个;并打一条日志。\n" +
        "> 真机实测(1.6MB + FLAC 105KB/s)≈ **9 秒**。选 15/20/25/30 秒会被钳到 9 秒 —— **不是档位失效**:实测把设备灌到 100% 后,设备立刻开始逐帧拒收(`Failed to send audio chunk`)并伴随失步,听感就是「卡一卡」。想让高档位真正生效,请在 ESPHome 里调大 `buffer_size`。\n" +
        "**起播延迟不随本值变化**(恒 ≈0.8 秒,由首帧锚点决定);卡顿把缓冲抽干后会自动回补到实际水位。\n" +
        "**进度条/歌词按「实际听到的位置」显示**,已扣除缓冲深度,不随本值偏移。\n" +
        "代价:**切歌间隙 ≈ 实际水位 − 0.8 秒**(曲末要等设备把缓冲播完)—— 档位越高,切歌停顿越明显。\n" +
...
- Encrypted spec-compliant clients stream after pairing (pairing flows landing progressively).`,
      fields: {
        ...
        prefill_buffer_ms: {
          ...
          help:
            "起播/卡顿恢复后,先把设备缓冲灌到这个水位再按实时速率推 —— 这就是设备能吸收的抖动与停顿窗口,越大越抗卡顿。\n" +
            "**本值是「期望水位」,实际水位还会按设备自己宣告的缓冲容量自动钳制**:服务端读 `client/hello` 里的 `buffer_capacity`(就是 ESPHome 的 `buffer_size`,单位字节,真机实测 1.6MB),除以实测压缩码率得到设备装得下的最长秒数,取更小的那个;并提示一条日志。\n" +
            "> 例:设备 1.6MB + FLAC 实测 ≈ 十来秒。此时选 20/25/30 秒会被钳到那个秒数 —— **不是档位失效**,超出部分设备放不下,推过去只会堆在服务端内存并抽干设备缓冲,反而变成「卡一卡」。想让高档位真正生效,请在 ESPHome 里调大 `buffer_size`。\n" +
            "**起播延迟不随本值变化**(恒 ≈0.8 秒,由首帧锚点决定);卡顿把缓冲抽干后会自动回补到实际水位。\n" +
            "**进度条/歌词按「实际听到的位置」显示**,已扣除缓冲深度,不随本值偏移。\n" +
            "代价:**切歌间隙 ≈ 实际水位 − 0.8 秒**(曲末要等设备把缓冲播完)—— 档位越高,切歌停顿越明显。\n" +
            "**Web 改完立即生效**(无需重启、不中断当前播放);只在当前这首歌上最多延迟 5 秒生效。",
