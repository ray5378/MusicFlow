...
|---|---|---|---|---|
| **Sendspin**（服务端推流给 ESP32） | ✅ **A 已做**（`1bcd473`） | 🚧 B 在做 | ⬜ 未做 | ⬜ 未做 |
...
1. **回环节点区间缓存**（主方案，对 ffmpeg 透明）
   ...
   - 首个 miss → 按**对齐块**（**256KB 量级**，实测 4MB 过大，见 §6.3.1）向上游拉一次，落本地缓存；
   - ffmpeg 那 8~19 次开放式 Range **全部命中缓存** ⇒ 上游往返 **9 → 1~2**；
   - **seek 前先按「目标时间 → 估算字节位置」预热该窗口**（这是把冷态也从 3.9s 降下来的关键）；
   - 缓存落 `DATA_DIR/cache/raw/`，键含 **url 指纹 + size**（web 行换源后 url 会变，不能只按 rowId/歌名），LRU 定额。
   - ⚠️ 响应必须**有界**（单次最多回一块）；缓存服务与 ffmpeg 必须能**共用事件循环**（见 §6.3.1 踩坑 1、2）。
2. **本地文件路径输入**（可选加速）
...
> - **B / C / D 同样未落地**（B 是共用管道，做了才轮到它们一起受益）。
>
> 也就是说：本节结论是「**能** + **该怎么做**」，**不是**「已经做了」。落地状态见 §4.1。
