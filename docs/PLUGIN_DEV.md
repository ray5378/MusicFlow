...
### 3.3 并行执行声明（`longRunning`）—— 批量任务并发

> 宿主把所有批量任务（同步/导入/推荐/匹配/清理/刮削）收进**全局队列**，默认并发上限 = 1（FIFO 串行，全部任务排队）。**只要插件声明了 `longRunning`**（存在有键的长耗时预算对象），宿主就在配置页自动注入「允许并行执行」开关（键 `batchParallel`，默认**关**）：
>
> - **关（默认）**：本插件的批量任务始终参与全局队列串行执行，不提升并发。
> - **开**：本插件的批量 worker 被计入全局并发上限，可与**其它开启本开关的插件**并行执行（利用多核，更快但 CPU 占用更高）。
>
> 与 §3.2 `schedules` 按「能力清单」注入不同，并行开关按「`longRunning` 是否有键」注入，同样发生在注册唯一漏斗 `registerPlugin()`（→ `withBatchParallelField`），**内置与外置插件统一覆盖**，无需手写。开关切换无需重启,保存即生效（`batchPacer` 并发上限实时联动,幂等）。
