...
| `description`                                            | ⬜  | 描述。                                                                                                                                                                                                                                                                                                                |
| `i18n` ⭐                                                 | ⬜  | **插件侧 i18n 字典**：`{ zh?, en? }`，按界面语言覆盖 `name / description / platformLabels / groups / configSchema` 各字段的 `label / help / options` 文案（见 §3.4）。默认文案即中文，故 **zh 可省略、只须补 en**；未声明语言或缺词自动回退默认文案，完全向后兼容。                                                                                                                 |
...
### 3.4 插件侧 i18n 字典（`i18n`）—— 界面语言下的多语文案

> 默认文案即中文，故插件只需在 `i18n.en` 里**补英文**；前端按当前界面语言（`zh-CN` / `en-US`）取用，未覆盖的关键字（含中文小语种）自动回退 manifest 默认文案。缺声明语言（如只写了 `en`）或缺 `key` 同样回退默认——**完全向后兼容**，老插件不写 `i18n` 走纯默认。

```js
{
  type: "source",
  name: "示例源",                      // 可作为 en 缺失时的回退文案
  description: "默认(中文)描述",
  configSchema: [{ key: "apiKey", label: "密钥", type: "text", help: "必填" }],
  i18n: {
    en: {
      name: "Example Source",
      description: "English description",
      platformLabels: { netease: "Netease", qq: "QQ Music" }, // source 插件平台映射
      groups: { schedule: "Scheduling", backend: "Backend" }, // 覆盖配置分组标题
      fields: {
        apiKey: { label: "API Key", help: "Required" },
        // 带 options 的字段可按 option value 覆盖单项文案
        platform: { label: "Platform", options: { netease: "Netease", qq: "QQ Music" } },
      },
    },
  },
}
...
- `i18n.zh` 可选（默认即中文），多用于把**某个外置插件原本的英文默认文案**翻回中文；内置插件一般只写 `en`。

- 分组标题优先级：`i18n.groups[key]` > 核心内置分组翻译 > 分组名 key。

- 前端取用逻辑在 `frontend/src/utils/pluginI18n.ts`（`resolvePluginI18n` / `resolveField` / `localName` / `localDesc`），随 `i18n.global.locale` 响应式重算，切换语言即时更新配置表单。

- 字段合并保留 `key / type / default / group` 等非文案字段，仅覆盖 `label / help / options`。
