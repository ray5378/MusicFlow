...
>
> **⏳ 快照口径（2026-09-19 标注）**：本文是**真机事实版**，最后更新 2026-09-18，对应主项目 v3.0.31–v3.0.34 阶段、ESPHome 2026.9.0 真机（ESP32-S3 `esp32-player-meet`）。此后 Sendspin 运行时已整体移入常驻子进程（v3.0.34 起；v3.0.39 起改用通用宿主 `services/rendererHost/`），但**设备侧（ESP32 + sendspin-cpp / micro-flac）的约束与判读方法依旧有效**。
