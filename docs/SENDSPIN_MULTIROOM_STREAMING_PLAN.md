...
### 2.5 解码内存对照结论（流式落地后 3.0.36；窗口 2026-09-19 由 60 收到 30 秒）
...
- 窗口上限取 **30 秒**，与 MA 的 `sleep_to_limit_buffer(30秒)` 对齐
  （`WINDOW_HIGH_SEC` 60→30，2026-09-19 定）：seek 回跳落到窗口外的几率上升，
  代价是按 `-ss` 重起 ffmpeg，越界频率在 240 soak 里继续观察；收窄后
  F32 上限 ~23MB → **~11.5MB**（＋5 秒历史环 ~2MB）。结构上两侧本就同构，
  参数现在也齐平了。
...
- 固定开销两侧都有且压不掉（Node/Python 运行时上百 MB、libFLAC asm 堆、
  ffmpeg 进程本身），F32 缓冲这 ~13MB 在整体里不是主要项；实测进程 RSS 应同档
  （~120MB 级）。
...
- **生产者**：每首歌一个长命 ffmpeg
  ...
  后台 reader 持续排入窗口。水位：低 20 秒 / 高 30 秒；满则停读
  （ffmpeg 被管道憋住，天然背压，无需额外协议），低于低水位续读。
- **内存上限**：窗口 30 秒 ≈ 11.5MB ＋ ffmpeg 常驻 ~15MB ＋ libFLAC 堆（固定）。
