...
> 状态：**A + B 全部完工** —— A（流式解码）已合入 main；B（组管理）核心层 + 路由增量口 + 扇出 + 前端 + 测试 T1–T7 全绿，**已随 v3.0.36 发版**。下文「进度与交接」保留为当时的施工日志。
...
### 施工清单（T1–T7 **已全部完成**；下方 6 条为起草时的顺序，与上方日志逐条对应）
...
6. 全量回归＋240 soak（内存曲线＋双 ESP32 加减成员演练）后再定发版。**已发版 v3.0.36**；soak 属真机 / 线上观察项，未阻塞发版。
...
| 内存 | ~122MB/320秒歌，随曲长线性涨，切歌重叠 ×2 | ~26MB 封顶，和曲长无关 |
...
### 2.5 解码内存对照结论（流式落地后 3.0.36；窗口 2026-09-19 60→30，2026-09-23 P0 对齐 MA BALANCED 升到 300 秒）

| | MA 上游 | 我们（流式开关开时） |
|---|---|---|
| 有界量 | 队列 6.4 秒＋背压 60 秒＋历史 1 秒 ≈ 67 秒 | 窗口 300 秒＋历史 5 秒 ≈ 305 秒 |
| F32 缓冲 | ≈ **26MB** | ≈ **115MB**（＋ffmpeg 常驻 ~15MB，两边都有） |
| 与曲长关系 | 无关（O(1)） | 无关（O(1)，短于窗口的曲整首驻留） |

- 解码窗口现取 **300 秒**（`WINDOW_HIGH_SEC=300` / `LOW=290`，滞回 10s），
  对齐 MA `AudioBuffer.max_size_seconds` BALANCED=300（`BUFFER_SIZE_MAP`），
  而非 MA 发送侧 `_PRODUCER_BUFFER_LIMIT_US`（60 秒）——两者是不同层：
  我们 300s 是**服务端 PCM 环容量**，MA 60s 是**发送侧推流背压**。
  代价是未消费前沿最多 ~115MB PCM（Buffer 外部内存），240 soak 看 RSS。
- 历史：60→30（2026-09-19，与当时 MA 30s 齐平）→ 300（2026-09-23 P0，
  seek 回跳在 5 分钟曲内几乎总能命中窗口）。
...
- 固定开销两侧都有且压不掉（Node/Python 运行时上百 MB、libFLAC asm 堆、
  ffmpeg 进程本身）；窗口 F32 ~115MB 属 Buffer 外部内存，不占 V8 老生代。
...
- **生产者**：每首歌一个长命 ffmpeg
  ...
  后台 reader 持续排入窗口。水位：低 290 秒 / 高 300 秒；满则停读
  （ffmpeg 被管道憋住，天然背压，无需额外协议），低于低水位续读。
- **内存上限**：窗口 300 秒 ≈ 115MB ＋ ffmpeg 常驻 ~15MB ＋ libFLAC 堆（固定）。
