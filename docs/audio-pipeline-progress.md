...
创建：2026-09-20 ｜ 最后更新：**2026-09-24（补「③ 段 DSP 接入 Sendspin」+ 音量改设备命令，未发版）** ｜ 已发版 **v4.0.0**（`be7a964`）
...
> **③ 段的六个出流出口现已全部接齐**（2026-09-24 补齐最后一处）：HTTP / Web（`resolveRequestAf`）、
> flow `resolveFlowAf`、DLNA、AirPlay，以及 **Sendspin**（`streamSource.resolveSendspinAf` ←
> `streamEngine.GroupPump.groupDspFilters`）。**Sendspin 之前是漏接**（`resolveSendspinAf` 只算了
> ②响度 + ⑤限制器），不是被「成组时成员音色停用」的规则挡住 —— 成组停用仍由
> `playerDspFilters` 内的 `isPeerGrouped` 独判，接线不重复实现该判定。详见 `docs/sendspin-权威方案文档.md` §2.16。
> ⚠️ 顺带钉死一处语义边界：`resolveLoudnessAf` 的 `enabled:false` / `escapeEnvVar=0` 会**整条返回空**
> （③ 段 DSP 一并丢弃），而用户可见的「音量归一化」开关走 `normalization` 字段、**只摘 ② 段保留 ③⑤** —— 两者方向相反，别混用。
...
| 2026-09-21 | （复核 · 续） | **播放结束判定按 MA 补齐第 2·3 层防御 → 修「唱完还停着约 10s」**。上一轮补的是第 ① 层（时长判据），但判定仍只在设备采样点跑：本仓 fallback poll 是 5s（`QueueController:217`），而 DLNA 位置外推**只在采样点推进**（`dlna/control.ts:1141`），于是「位置到时长 + `END_GRACE_MS` 8s」被采样粒度拖成 ~10s。①`PlaybackTracker` 新增 `tick(nowMs)`：用「末次 PLAYING 快照 + 墙上时钟」自行外推，复用同一套「到点 + 持续宽限」判据；只读不写（不动 `prev`，否则污染状态迁移比较），非 PLAYING 不推进（暂停期间墙钟不算进宽限），判完清时长防重复 advance。②`PlayerController` 新增 `startOverrunTicker()`（500ms，对照 MA `players/controller.py:_poll_players:3202-3216` 的 0.5s `corrected_elapsed_time` 推送），**不触设备**；派发前先清该 player 的去抖窗口，否则窗口内后到的 reportState 会用 `none` 覆盖这条决策。③`index.ts` 与 `startPollLoop` 并排启动。新增 9 例（tracker 6 + controller 3）；两道守卫做负向验证：破坏 PLAYING 守卫精确变红 2 例、破坏宽限守卫精确变红 5 例。 |
| 2026-09-24 | （本轮） | **音量改设备命令 + ③ 段音色接入 Sendspin（两条 Sendspin 收口，未发版）**。① **音量/静音改走 `server/command`**（`sendspin/server.ts` + `playerCore.ts`）：此前 `appliedGain = conn.volume × 组音量 / 100` 经 `scalePcm` **烘进编码后的 PCM**，而设备缓冲（档位 30s 时最明显）里全是旧增益 ⇒ 拖完群组音量要等设备把缓冲播完才生效。**对照 MA**（`music_assistant/providers/sendspin`）：**只有响度归一化增益烘进 PCM**（`controllers/player_queues/config.py:462` 的 "(re)baked gain filter"，且**只在新建流时算一次**），**用户/组音量与静音走 `server/command`**（`player/v1.py:600-616` 发 `player.command="volume"`）在设备**输出级**实时施加 —— MA player 的 audio transformer 只再导出编码器（`FlacEncoder`/`OpusEncoder`/`PcmPassthrough`）、**不含任何 gain**，这才是「MA 改音量立即生效」的全部原因。**落地**：`parseHelloSupportedCommands()` 读 `client/hello` 的 `player@v1_support.supported_commands`（兼容 `player_support` / 顶层键名，只认 spec 允许的 `volume`/`mute`），**两者都宣告**才启用（只宣告 `volume` 时静音无法可靠表达 —— 要么恒发 `volume=0` 把设备自己那把旋钮拧掉、要么静音无效，那正是要避开的「看着生效、实际打架」）；`appliedGain()` 对能力设备恒 **unity 100**、否则回退旧乘积（未宣告能力的老固件/legacy 客户端**逐字节不变**）；`syncVolumeTo()` 先 volume 后 mute（顺序即语义：取消静音那一拍设备拿到的是新音量，不闪旧值）；对齐时机全部收敛到 `SendspinServer.syncVolume()` 一处 —— **5 个入口**：改音量/静音（`setVolumeCore`/`setMutedCore`，in-proc 与 fork 子进程同咽喉）、**成员入组**（`SendspinGroup.add`，否则「老成员停在旧值、新成员停在默认 100」）、设备上线/重连（`onConnectionActivated`，此时 hello 已解析完）、重启后持久恢复、TTS 播报结束还原（那是直接赋值、绕过 setVolumeCore，必须显式补发）。② **③ 段音色 DSP 接入 Sendspin 流** —— 经核查**是漏接**、不是被成组规则挡住：`WindowSource` 增 `dspFilters?: string[]`（纯音源层不碰 peerId→配置映射），`resolveSendspinAf` 把片段交给 `resolveLoudnessAf({extraFilters})`，**顺序 ②→③→⑤ 由那一个函数单点保证**（本函数只传不拼）；`GroupPump.groupDspFilters()` 在起播时算一次（改配置**下一首生效**），peerId 由 `dspPeerIdForGroup()` 翻译（用户组 `ug:<gid>`→`group:<gid>`，与前端「音色」下拉的群组条目同形；单设备组→`sendspin:<clientId>`）；**成组时成员音色自动停用**仍由 `playerDspFilters` 内的 `isPeerGrouped` **独判**，接线不重复实现该规则。走动态 `import("../playerDsp.js")`（避免模块加载期把组管理器链绑死）、注入音源直接返回 `[]`、读失败回退 `[]`（音色是可选装饰，绝不能因为它让整组放不出声）。③ **前端配套**：移除群组页（`frontend/src/views/Groups/index.vue`）成员行的「成员迷你音量条」（静音键 + 滑条 + 数值）—— 群组推流按 `ug:<gid>` 组对象的 volume 走（全组同一份 PCM、同一编码增益），而那个滑条打的是 `/v1/peers/sendspin:<id>/volume`、写的是该设备**自己那个单设备组**的 volume ⇒ **群组播放时压根不参与，是「看着能动、实际没作用」的死控件**；成员静音同理交回群组级入口，避免「组里两台设备各有一把旋钮」的错误心智模型。3 个配套 i18n key 一并删除（`zh`/`en` 仍 **1362 键对齐**）。**测试**：新增 `backend/tests/services/sendspinVolumeCommand.test.ts` **9 例**（hello 解析的真机帧形/legacy 键名/顶层键/非法值过滤与去重、unity 增益分派、未宣告命令**一条都不发**、只宣告 `volume` 也回退、入组即对齐、静音的独立旋钮语义、**真机链路** —— 真实 legacy WS 连接上确实收到 `server/command`；另加 `SendspinGroup` 纯层 6 例）；`streamSource.test.ts` **+7 例**（②→③→⑤ 顺序、多段 DSP 原序、无配置时与接线前逐字节一致、逃生舱优先级高于 DSP、`dspPeerIdForGroup` 三态）。**两条踩坑写进测试注释**：① `vitest.config.ts` 开了 `sequence.shuffle` ⇒ **每个用例必须自建基线**，且观测窗口顺序必须「先清空帧 → 再造基线 → 等基线到齐 → 再清空」（先清再等会漏掉**在途帧**，它们在清空之后才落地、污染下一次观测）；② 单测必须往 `plugins` 表写 `auto_discover:false`，否则会经 mDNS **真的去拨局域网里的音箱**。**门禁**：`tsc --noEmit` 0、`check-i18n` 1362 键对齐 0、前端 `vue-tsc && vite build` 通过；`sendspin` 全组 + 新测试 **225 例**通过。文档同步：`docs/sendspin-权威方案文档.md` 新增 **§2.15/§2.16** 并回标 §2.6③（`gain` 维度收窄为恒定 unity）。**未发版**（无 tag/Release）。 |
