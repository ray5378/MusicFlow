...
# 第十一部分：SENDSPIN 修复经验方法总纲

> 本部分是对前 10 部分的「操作手册」提炼：**下次遇到 sendspin 同步 / 卡顿 / 延迟 / 失步类问题，照这套流程走，不要凭空推理。**
> 核心教训来自 v4.0.20 的两例修复（§5.3 / §2.6 / §8.3）以及两个惨痛坑 **B14 / B15**。本部分本身也是 v4.0.20 这轮修复的收尾产物。

## 11.0 大前提：动手前必须先读 MA，再对照我们自身代码

**任何 sendspin 同步 / 播放问题，第一步不是改代码，而是先读 Music Assistant 的参考实现（`aiosendspin`），再逐行对照我们自己的代码找分叉点。**

理由（血泪）：

- MA 的同步内核在 `aiosendspin`（节奏 / 背压 / late-join 全在那），provider（`music_assistant/providers/sendspin/*.py`）只做 DSP / 元数据 / 发现 / 桥接。我们**没有**同步内核，全是自己写的 → 出问题时「我们的代码」就是第一嫌疑人，而 MA 是「标准答案」。
- 这一轮的两个坑都源于「没把 MA 读全 / 没对照全」：
  - **坑 B15**：每成员一个编码器 + 每批产出取 max → 把两个块编码器的**并集**当成时间线推进量 → 群组播放速率 1.44×、整组卡顿。MA 用 `TransformerPool` 按 transform key 共享编码器（§8.2 #8），我们本该**按 `(codec,gain)` 分组编码、时间线取 max-of-累计**。
  - **坑 B14**：曾凭「对齐不需要历史」一个正确结论，**否掉**了「出声需要回填」这个同样正确的问题 → 文档里写过一段错的「不抄 MA 式回填」。MA 的 `on_role_join` 靠缓存回放尚未播到的 chunk（`LATE_JOINER_MIN_LEAD_US=100ms`）。
- 结论：**先读 MA 对应模块（§8.0 速查表），把它的机制与常量列成对齐表（§8.1 / §8.2），再去定位我们的分叉点**。绝不凭「感觉这个更稳」（坑 B9 选 PCM）、绝不凭「改个参数试试」（坑 A2 / A3）。

## 11.1 标准流程（8 步，照走）

1. **读参考实现，建对齐表**（大前提落地）
   - 打开 §8.0，定位问题对应的 aiosendspin 文件 / 符号（`push_stream.py` / `group.py` / `roles/` / `playback.py`）。
   - **通读该机制的完整链路**（不要只读一半），把常量、时序、分支列出来。
   - 与我们 `backend/src/services/sendspin/` 的对应代码逐条比对，标出分叉点。
2. **真机复现 + 受控 A/B**（§7.6）
   - 同一设备、同一首歌，只改一个变量（档位 / codec / 缓冲 / 组成员）。
   - 同时抓：服务端自证日志（§7.2）+ 设备 6053 日志（§7.3）。
3. **量化症状，定位根因**（用数不用感）
   - 卡顿 → 算群组播放速率（`pos` 增量 / 墙钟增量），1.44× 即「推进量被放大」。
   - 延迟 → 算新成员出声延迟（加入时刻 → 设备 `i2s_audio.speaker Starting`），29s 即「组游标领先墙钟一整个水位」。
   - 失步 → 数 `SYNC LOST` / `Lost sync` 计数，不是看有没有日志（稳定播放设备零日志，§7.2）。
4. **以 MA 为蓝本设计修复，不自创机制**
   - 每条修复都能在 §8.2 找到对应的 MA 机制与「✅ 已实现」标记。
   - 本仓三处等价实现：按 `(codec,gain)` 分组编码（§2.6③）、时间线 max-of-累计（§2.6）、`seedLateJoin` 回填 `recentByGroup`（§5.3）。
5. **单测锁死语义**（不是「跑通就行」）
   - 新增 `tests/sendspin/lateJoinBackfill.test.ts`（8 例）、`pushFrameGroupEncode.test.ts`（10 例）。
   - 覆盖：首帧不在过去、≈100ms 出声、截止于组游标、过期缓存不回填、`available:false` 不回填、容量钳制、按编码组取字节、成员数不影响推进量。
6. **真机验收（唯一有效判据 = 用户耳朵）**（§8.3）
   - 回填延迟 ≈0.1s、群组速率 1.00×、`SYNC LOST`=0、`timeline RE-anchored`=0（流全程未重启）。
   - 读关键日志：`target=882706us`（= `send_ahead 800000 + LATE_JOIN_MARGIN_US 100000`，证明贴住时间轴）、`chunks=341 span=29099ms`（= 钳制水位，反证编码器粒度 85.33ms/帧）。
7. **写进权威文档（闭环）**
   - 把「正确结论 + 被否掉的错结论 + 对齐表更新 + 真机读数」都落库（本次：§8.0 速查表、§5.3 反转、坑 B14 / B15、§8.3 验收）。
8. **发版（230 → GitHub，CI 防线）**
   - 本地提交 → `git bundle` 中转推 230 → 230 `git push origin` + 打 `vX.Y.Z` tag → CI 自动跑（contract-gate / security / Version Guard / sync-to-gitee / build-and-push）→ Release 正文自动生成 + Docker 镜像产出。
   - 注意：`github.com` 本机常不可达，**发版一律走 230 中转**，不要在本机直推（详见本仓发版约定）。

## 11.2 不要怎么做（反面清单，来自坑 B14 / B15 / A5 / B10）

- ❌ **局部推理、用一条正确结论否掉另一条正确结论**（坑 B14）。读 MA 要读完整链路。
- ❌ **凭「感觉更稳」选方案**（坑 B9 选 PCM、「对齐 MA」改 `frame_size` 坑 A3）。一切以 MA 对齐表 + 真机计数为准。
- ❌ **「服务端日志全绿 = 成功」**（坑 A5）、**「单测全绿 = 真机不炸」**（坑 B10）。两者都只是门槛，最终判据是设备日志计数 + 用户耳朵。
- ❌ **时间线把「并集」当推进量**（坑 B15）。多编码器 / 多成员时务必分组编码 + max-of-累计。
- ❌ **改参数试运气**（坑 A2 / A3）。先读源码 / 抓字节对齐，再谈调参（§7.8）。

## 11.3 本仓仍挂账的分叉点（下一轮照 §11.1 走）
